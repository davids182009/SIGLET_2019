...
define([
  ...
  function(declare, _WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin,
     ...
     *
     */
    return declare("ContentCapas", [_WidgetBase, _TemplatedMixin,
      ...
      startup: function() {
        ...
        on(dom.byId('Lastcapa'), 'dragover', function(event) {
          ...
          setTimeout(function() {
            domConstruct.place(dom.byId('capaSpot'), nodoId,
              'before');
          }, 10);
        });
        this.nodoInputBuscar = query('input[type=text]', dom.byId(
          'formFiltrarCapa'))[0];
        on(dom.byId('formFiltrarCapa'), 'submit', lang.hitch(this, this
          .filtrarCapas));
      },
      /**
      ...
      __setupPanelLayout: function() {
        ...
        explorador = new ContentPane({
          ...
          content: '<div id="tree" class="nodoOculto"></div>' +
            ...
            'Cargando listado de capas espaciales …' + '</div>',
          minSize: 200
        });
        panel = new ContentPane({
          ...
          content: '<ul id="tituloAreadeTrabajo">' +
            ...
            '<li class="opciones prenderCapas">' + '<span>' +
            '<i class="icon ion-eye"></i>' + '</span>' + '</li>' +
            '<li class="opciones apagarCapas">' + '<span>' +
            '<i class="icon ion-eye-disabled"></i>' + '</span>' +
            '</li>' + '</ul>' + '<div id="workspace">' +
            '<div id="graphicLayer">' +
            '<div id="Lastcapa" draggable="true"></div>' + '</div>' +
            '<div id="layerSplitter">' + '</div>' +
            '<div id="imageLayer">' + '</div>' + '</div>',
          style: 'height: 300px;',
          splitter: true,
          minSize: 300
        });
      ...
      _setupTreeLayers: function() {
        ...
        request.get(
          ...
          for (let i = 0; i < tablaAdmonServ.length; i++) {
            jsonAjustado = this._OrganizarJSON(tablaAdmonServ[i],
              '0', jsonAjustado);
          }
          ...
          domClass.add(dom.byId('LayerExplorerMask1'),
            'nodoOculto');
                ...
                cb.on("change", function(isChecked) {
                  if (isChecked) {
                    ...
                    if (args.item.TIPO == 'REST')
                      capaWidget.placeAt(dom.byId(
                        "graphicLayer"), "first");
                    else {
                      capaWidget.placeAt(dom.byId(
                        "imageLayer"), "first");
                      domAttr.set(capaWidget.id, {
                        'draggable': false
                      });
                    }
                  } else {
      ...
      _OrganizarJSON: function(datos, padre, jsonAjustado) {
        ...
        for (let i = 0; i < tematicas.length; i++) {
          jsonAjustado = this._OrganizarJSON(tematicas[i],
            idNodoTematica, jsonAjustado);
        }
        //RECORRER CAPAS
        let capas = datos.capas
        for (let i = 0; i < capas.length; i++) {
          if (capas[i].tipoServicio == 'REST' || capas[i].tipoServicio ==
            'WMS') {
            nodoObj = {
      ...
      apagarCapas: function() {
        let listCapaWidget = registry.findWidgets(dom.byId(
          'graphicLayer'));
      ...
      filtrarCapas: function(event) {
        ...
        if (this.tree != null) {
          ...
          let listaResultado = this.capasStore.query(function(nodo) {
            let re = new RegExp('[a-z0-9]*(' + parametro.toUpperCase() +
              ')[a-z0-9]*');
          ...
          if (listaResultado.length == 0) {
            this.mostrarMensaje({
              ...
              content: 'Parámetro de búsqueda “' + parametro +
          ...
          for (let i = 0; i < listaResultado.length; i++) {
            ...
            while (parametroBusqueda != '0') {
              ...
              if (parametroBusqueda != '0') {
                nodoTargetBusqueda = this.capasStore.query({
      ...
      addFromFilePane: function(layer, job) {
        ...
        widget = new CapaWidget(cp).placeAt(dom.byId("graphicLayer"),
          "first");
      ...
      addFromUrlPane: function(layer, job) {
        ...
        if (job.type == 'WMS') {
          ...
          widget = new CapaWidget(cp).placeAt(dom.byId("imageLayer"),
            "first");
        } else {
      ...
      addResultadoGeoProceso: function(layer, job) {
        ...
        widget = new CapaWidget(cp);
        widget.placeAt(dom.byId("graphicLayer"), "first");
        return widget;
      },
      /**
